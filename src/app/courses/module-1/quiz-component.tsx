...
import { submitQuizScore, getLeaderboard } from "../actions"
...
export function GamifiedQuiz({ moduleId }: { moduleId: string }) {
  ...
  const handleNext = async () => {
    if (currentQuestion < questions.length - 1) {
      ...
      await submitQuizScore(moduleId, score + (isCorrect ? 100 : 0)) // include last question score if correct
  ...
  // Pre-fetch leaderboard if we want, or just wait till end.
    ...
    return (
      <div className="flex justify-center mt-8">
        <Button 
          ...
          className="bg-primary hover:bg-primary/90 text-primary-foreground text-lg py-6 px-10 rounded-full shadow-lg"
        >
  ...
  if (completed) {
    return (
      <Card className="border-primary/20 shadow-xl scroll-mt-20 overflow-hidden" id="leaderboard-section">
        <div className="bg-gradient-to-r from-primary to-primary/80 p-8 text-center text-white">
          ...
          <h2 className="text-3xl font-bold mb-2">¡Reto Completado!</h2>
          <p className="text-primary-foreground/80 text-lg">Tu puntaje final es:</p>
          <div className="text-6xl font-black mt-2 tracking-tighter">{score + (isCorrect ? 100 : 0)} <span className="text-2xl font-medium tracking-normal text-white/70">pts</span></div>
        </div>

        <CardContent className="p-0">
          <div className="bg-zinc-50 p-4 border-b">
            <h3 className="font-bold text-zinc-900 flex items-center justify-center">
              <Medal className="w-5 h-5 mr-2 text-zinc-500" /> Leaderboard Global - Top 10
            </h3>
          </div>
          
          <div className="p-6">
            {isSubmitting ? (
              <div className="text-center py-8 text-zinc-500 animate-pulse">Guardando tu puntaje y calculando posiciones...</div>
            ) : (
              <div className="space-y-3">
                {leaderboard.length === 0 ? (
                  ...
                  leaderboard.map((leader, i) => (
                    <div key={i} className={`flex items-center justify-between p-4 rounded-lg border ${i === 0 ? 'bg-yellow-50 border-yellow-200' : i === 1 ? 'bg-zinc-100 border-zinc-200' : i === 2 ? 'bg-orange-50 border-orange-200' : 'bg-white'}`}>
                      <div className="flex items-center gap-4">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${i === 0 ? 'bg-yellow-400 text-yellow-900' : i === 1 ? 'bg-zinc-300 text-zinc-800' : i === 2 ? 'bg-orange-300 text-orange-900' : 'bg-zinc-100 text-zinc-500'}`}>
                          {leader.position}
                        </div>
        ...
        <CardFooter className="bg-zinc-50 p-6 border-t flex justify-center">
          <Button onClick={() => window.location.href = "/courses"} size="lg" className="w-full sm:w-auto">
            Continuar a Mis Cursos
          </Button>
        </CardFooter>
      </Card>
    )
  ...
  return (
    <Card className="border-primary/20 shadow-md scroll-mt-20 relative overflow-hidden" id="quiz-section">
      ...
      <CardHeader className="bg-primary/5 pb-4 border-b border-primary/10 pt-8">
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="bg-white px-4 py-1.5 rounded-full shadow-sm border font-bold text-primary">
            {score} pts
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-6 md:p-10">
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {questions[currentQuestion].options.map((opt, i) => (
              <button
                ...
                className={`w-full text-left p-6 rounded-xl border-2 transition-all shadow-sm flex items-center ${
                  selectedAnswer === i 
                    ? 'border-primary bg-primary/5 text-primary-foreground transform scale-[1.02]' 
                    : 'border-zinc-200 hover:border-zinc-300 hover:bg-zinc-50 bg-white text-zinc-700'
                ...
                `}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-4 font-bold shrink-0 ${selectedAnswer === i ? 'bg-primary text-white' : 'bg-zinc-100 text-zinc-500'}`}>
          ...
          {/* Feedback */}
          {isCorrect !== null && (
            <div className={`p-6 rounded-xl flex items-start gap-4 border ${isCorrect ? 'bg-green-50 border-green-200 text-green-900' : 'bg-red-50 border-red-200 text-red-900'} animate-in fade-in slide-in-from-bottom-4`}>
              ...
              <div>
                ...
                <p className="text-base opacity-90">{questions[currentQuestion].explanation}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <CardFooter className="bg-zinc-50 p-6 border-t flex justify-end">
        {isCorrect === null ? (
          <Button size="lg" onClick={handleAnswerSubmit} disabled={selectedAnswer === null} className="w-full md:w-auto px-8">
            Verificar Respuesta
          </Button>
        ) : (
          <Button size="lg" onClick={handleNext} className="w-full md:w-auto px-8 bg-zinc-900 hover:bg-zinc-800 text-white">
            {currentQuestion < questions.length - 1 ? 'Siguiente Pregunta' : 'Finalizar y Ver Ranking'} <ChevronRight className="w-5 h-5 ml-2" />
          </Button>
        )}
      </CardFooter>
    </Card>
